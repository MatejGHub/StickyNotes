  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function setCopiedState(button) {
        button.textContent = '<?php echo esc_js(__('Copied!', 'sticky-comment')); ?>';
        setTimeout(function() {
          button.textContent = '<?php echo esc_js(__('Copy link', 'sticky-comment')); ?>';
        }, 2000);
      }

      function showManualCopyPrompt(url) {
        window.prompt('<?php echo esc_js(__('Copy this link manually:', 'sticky-comment')); ?>', url);
      }
...
function render_sticky_notes_table()
{
  ...
  if ($notes) {
    ...
    if (!is_array($hidden_columns_global)) {
      $hidden_columns_global = array('created');
    }
